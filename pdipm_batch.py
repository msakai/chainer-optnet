...
def batch_lu_factor(A):
    ...
    if cupy_available and xp == cupy:
        ...
        Ps = []
        for i in range(len(A)):
            A[i], Ps[i] = scipy.linalg.lu_factor(A[i], overwrite_a=True)
            Ps.append(piv)
...
def forward(Q, p, G, h, A, b, kkt_solver: KKTSolver,
    ...
    for i in range(maxIter):
        ...
        if verbose >= 1:
            if cupy_available and xp == cupy:
                print('iter: {}, pri_resid: {:.5g}, dual_resid: {:.5g}, mu: {:.5g}'.format(
                    i, pri_resid.mean().get(), dual_resid.mean().get(), mu.mean().get()),
                      file=sys.stderr)                
            else:
                print('iter: {}, pri_resid: {:.5g}, dual_resid: {:.5g}, mu: {:.5g}'.format(
        ...
        if best['resids'] is None:
            ...
            best['z'] = xp.copy(z)
            best['s'] = xp.copy(s)
            best['y'] = xp.copy(y) if y is not None else None
            nNotImproved = 0
        else:
